/* =============================================
   Aplicación de Tráfico Simulado - Lógica JS
   ============================================= */
...
// Coordenadas iniciales (San Marcos, El Salvador)
...
// Horas hasta que un reporte expira automáticamente, según su tipo
...
const AGENT_LOCATION_STALE_MS = 2 * 60 * 1000; // 2 minutos sin actualizar = se considera desconectado

// --- Cálculo de Rutas ---
let isRoutingMode = false;
let routePendingFrom = null; // {lat, lng} del primer punto tocado en modo "Trazar ruta"
let routeLine = null;
...
let routeAlternatives = []; // [{distance, duration, latlngs}]
let selectedRouteIndex = 0;
let alternativeRouteLines = []; // líneas grises de las rutas no elegidas
let lastRouteFrom = null;
let lastRouteTo = null;

// --- Retraso por tráfico en vivo en rutas (Google Directions, opcional) ---
...
let navigationDestination = null; // {lat, lng}
let lastNavRecalcAt = 0;
let lastNavTrafficCheckAt = 0;
let lastNavTrafficExtraSeconds = 0;
const NAVIGATION_ARRIVAL_METERS = 30;
const NAVIGATION_RECALC_MS = 10000; // recalcular distancia/tiempo restante (OSRM, gratis) cada 10s
const NAVIGATION_TRAFFIC_RECALC_MS = 60000; // volver a preguntarle a Google el retraso por tráfico como máximo cada 60s (tiene costo)

// =============================================
// Inicialización del Mapa
// =============================================
function initMap() {
    // Crear el mapa centrado en las coordenadas por defecto, zoom 14
    map = L.map('map', {
        zoomControl: false // Ocultamos el control por defecto
    ...
    // Añadir capa base inicial (estándar)
    currentBaseLayer = baseLayers.standard;
    currentBaseLayer.addTo(map);

    // Capa visual de tráfico (Google)
    ...
    // Cluster de puntos de control (evita que el mapa colapse con muchos marcadores)
    controlPointsCluster = L.markerClusterGroup({ maxClusterRadius: 60 });
    map.addLayer(controlPointsCluster);

    // Recargar los puntos visibles cada vez que el usuario mueve/hace zoom al mapa
    map.on('moveend', function() {
        clearTimeout(viewportLoadTimeout);
        viewportLoadTimeout = setTimeout(loadControlPointsInView, 400);
    });

    // Evento click en el mapa: trazar ruta, registrar punto de control o cerrar paneles
...
// =============================================
// Autenticación (Supabase Auth) y Bloqueo de Acceso
// =============================================
...
async function initAuth() {
    ...
    supabaseClient.auth.onAuthStateChange(async (event, session) => {
        ...
        if (currentUser) {
            ...
            if (wasLoggedIn) {
                // Se cerró sesión estando dentro de la app: recargar para volver al bloqueo limpio
                location.reload();
                return;
            }
        }
...
// Se llama una sola vez, cuando se confirma que hay sesión iniciada
function startApp() {
...
async function submitGateAuthForm(event) {
    ...
    // Si el login fue exitoso de inmediato, onAuthStateChange oculta el bloqueo e inicia la app
}
...
async function submitCreateUser(event) {
    ...
    // Cliente aparte, sin persistir sesión, para no cerrar la sesión del admin
...
async function loadControlPointsInView() {
    ...
    // Quitar del cluster los puntos que quedaron fuera del área visible
    ...
    // Agregar los puntos nuevos que entraron al área visible
    data.forEach(point => {
...
function toggleBoundaryLayer() {
    ...
    if (!boundaryLayer) return; // todavía se está cargando; loadBoundaryLayer() respeta isBoundaryVisible al terminar
...
// Dejar de transmitir la ubicación del agente si cierra/recarga la pestaña (best-effort)
...
// PWA: registrar el service worker (permite instalar la app y que cargue rápido/offline)
